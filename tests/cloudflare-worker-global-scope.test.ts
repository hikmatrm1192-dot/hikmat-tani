 ...
 *
 * Memverifikasi global-scope safety, lazy service initialization, and the
 * Worker auth/health lifecycle.
 */
...
async function runWorkerGlobalScopeTests() {
  ...
  assert(
    healthProbeCount === 1,
    '5. Worker Health Check: /api/v1/health benar-benar melakukan probe SELECT 1 ke D1'
  );
  ...
  assert(
    ...
    '6. Worker Register Endpoint: Mengembalikan HTTP 201 dengan struktur token & farmer profile valid'
  );
  ...
  assert(
    ...
    '7. Anti-Duplicate NIK: Registrasi ganda ditolak dengan status HTTP 409'
  );
  ...
  assert(
    loginRes.status === 200 && loginJson.success === true && typeof loginJson.data?.token === 'string',
    '8. Worker Login Endpoint: Login via Nomor HP + PIN berhasil dan mengembalikan session token'
  );
  ...
  assert(
    meRes.status === 200 && meJson.success === true && meJson.data?.farmer?.name === 'Budi Santoso',
    '9. Worker Profile Endpoint: /api/v1/auth/me mengembalikan profil petani terotentikasi'
  );
  ...
  assert(logoutRes.status === 200, '10. Worker Logout Endpoint: /api/v1/auth/logout berhasil mengakhiri sesi');

  const managerResult = await adminService.authenticateAdmin('manager_test', 'test-password', mockEnv.DB);
  assert(
    managerResult === null || typeof managerResult === 'object',
    '11. Admin Lazy Auth: Autentikasi Pengelola (MANAGER) berfungsi dengan baik saat runtime'
  );
