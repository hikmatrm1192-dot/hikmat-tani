...
export async function runOptPhotoIntegrationTests() {
  ...
  // Skenario 2: Pengamatan manual + foto jelas
  {
  ...
  // Skenario 3: Opsi "Belum tahu" + foto tidak menghasilkan diagnosis palsu
  {
    ...
    const hasBlas = results.some((r) => r.opt.id === 'opt-blas-padi');
    assert(hasBlas, 'Skenario 3: Harus menyertakan Penyakit Blas Padi sebagai kandidat pembanding');
  ...
  // Skenario 5: PHT 7 langkah / 4 pilar disajikan lebih dulu
  {
  ...
  // Skenario 6: Bahan aktif kimia berakar dari master data resmi
  {
  ...
  // Skenario 10: Eliminasi stopword dan noise non-agronomi
  {
  ...
  // Skenario 12: Offline-first dan determinisme
  {
  ...
  // Skenario 13: Pencarian tanpa kata kunci (query kosong) mengembalikan katalog pustaka
  {
  ...
  // Skenario 14: Regresi HST dan engine nutrisi tidak terganggu
  {
