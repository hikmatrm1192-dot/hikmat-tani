  ...
  async fetch(request: Request, env: Env, _ctx: any): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/v1/admin/')) {
        ...
        const adminAuth = authenticateAdminOnWorker;
        ...
        if (authResult.status !== 200 || !authResult.body?.success) {
          return jsonResponse(authResult.body, authResult.status, corsHeaders);
        }
        ...
        try {
          if (url.pathname === '/api/v1/admin/config' && request.method === 'GET') {
            const configData = await adminService.getAdminConfigAsync();
          ...
          if (url.pathname === '/api/v1/admin/config' && request.method === 'PUT') {
            if (adminRole !== 'SUPER_ADMIN' && adminRole !== 'MANAGER') {
              return jsonResponse({ success: false, error: { code: 'FORBIDDEN', message: 'Akses ditolak.' } }, 403, corsHeaders);
            }
            const body = (await request.json().catch(() => ({}))) as any;
            const updated = await adminService.updateAdminConfigAsync(body, adminId, ipAddress);
          ...
          if (url.pathname === '/api/v1/admin/users' && request.method === 'GET') {
            ...
            const users = await adminService.listAdminUsersAsync();
            return jsonResponse({ success: true, data: users }, 200, corsHeaders);
          }

          if (url.pathname === '/api/v1/admin/users' && request.method === 'POST') {
            if (adminRole !== 'SUPER_ADMIN') {
              return jsonResponse({ success: false, error: { code: 'FORBIDDEN', message: 'Hanya SUPER_ADMIN yang dapat membuat akun pengelola.' } }, 403, corsHeaders);
            }
            const body = (await request.json().catch(() => ({}))) as any;
            const created = await adminService.createAdminUserAsync(body, adminId, ipAddress);
            return jsonResponse({ success: true, data: created }, 201, corsHeaders);
          }

          if (url.pathname.startsWith('/api/v1/admin/users/') && request.method === 'PUT') {
            if (adminRole !== 'SUPER_ADMIN') {
              return jsonResponse({ success: false, error: { code: 'FORBIDDEN', message: 'Hanya SUPER_ADMIN yang dapat mengubah akun pengelola.' } }, 403, corsHeaders);
            }
            const targetId = decodeURIComponent(url.pathname.split('/').pop() || '');
            const body = (await request.json().catch(() => ({}))) as any;
            const updated = await adminService.updateAdminUserAsync(targetId, body, adminId, ipAddress);
            return jsonResponse({ success: true, data: updated }, 200, corsHeaders);
          }

          if (url.pathname.startsWith('/api/v1/admin/users/') && request.method === 'DELETE') {
            if (adminRole !== 'SUPER_ADMIN') {
              return jsonResponse({ success: false, error: { code: 'FORBIDDEN', message: 'Hanya SUPER_ADMIN yang dapat menghapus akun pengelola.' } }, 403, corsHeaders);
            }
            const targetId = decodeURIComponent(url.pathname.split('/').pop() || '');
            const deleted = await adminService.deleteAdminUserAsync(targetId, adminId, ipAddress);
            return jsonResponse({ success: true, data: deleted }, 200, corsHeaders);
          }

          if (url.pathname === '/api/v1/admin/audit-logs' && request.method === 'GET') {
            if (adminRole !== 'SUPER_ADMIN') {
              return jsonResponse({ success: false, error: { code: 'FORBIDDEN', message: 'Hanya SUPER_ADMIN yang dapat melihat audit log.' } }, 403, corsHeaders);
            }
            const logs = await adminService.getAuditLogsAsync();
          ...
          return jsonResponse({
            success: false,
            error: { code: error?.code || 'ADMIN_ERROR', message: error?.message || 'Terjadi kesalahan pada area pengelola.' },
          }, error?.statusCode || 500, corsHeaders);
        }
      }
      ...
      if (url.pathname === '/api/v1/auth/register' && request.method === 'POST') {
        const body = (await request.json().catch(() => ({}))) as any;
        try {
          const result = await authService.registerFarmerAsync(body, env.DB);
          return jsonResponse(result.body, result.status, corsHeaders);
        } catch (error: any) {
      ...
      if (url.pathname === '/api/v1/auth/login' && request.method === 'POST') {
        const body = (await request.json().catch(() => ({}))) as any;
        try {
          const result = await authService.loginFarmerAsync(body, env.DB);
          return jsonResponse(result.body, result.status, corsHeaders);
        } catch (error: any) {
      ...
      if (url.pathname === '/api/v1/auth/me' && request.method === 'GET') {
        ...
        if (!session) {
          return jsonResponse({ success: false, error: { code: 'UNAUTHORIZED', message: 'Sesi tidak valid atau telah kedaluwarsa.' } }, 401, corsHeaders);
        }
        try {
      ...
      if (url.pathname === '/api/v1/sync/push' || url.pathname === '/api/v1/sync/pull') {
        ...
        try {
          ...
          if (url.pathname.endsWith('/push')) {
            ...
            const result = await db.sync.push(body, session.userId);
            return jsonResponse(result, 200, corsHeaders);
          }
          const cursor = url.searchParams.get('cursor');
          const result = await db.sync.pull(session.userId, cursor ? Number(cursor) : undefined);
          return jsonResponse(result, 200, corsHeaders);
        } catch (error: any) {
      ...
      if (url.pathname.startsWith('/api/')) {
        return jsonResponse({ success: false, error: { code: 'NOT_FOUND', message: 'Endpoint API tidak ditemukan.' } }, 404, corsHeaders);
      }
      ...
      return jsonResponse({
        success: false,
        error: { code: error?.code || 'INTERNAL_ERROR', message: error?.message || 'Terjadi kesalahan internal.' },
      }, error?.statusCode || 500, corsHeaders);
    }
  },
