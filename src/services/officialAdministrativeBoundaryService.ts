...
function ringToPoints(ring: unknown): LatLngPoint[] {
  ...
  return ring.filter((p) => Array.isArray(p) && p.length >= 2).map((p) => ({ lat: Number(p[1]), lng: Number(p[0]) })).filter((p) => Number.isFinite(p.lat) && Number.isFinite(p.lng));
}
...
function geometryToPoints(geometry: { type: string; coordinates: unknown } | null): LatLngPoint[] {
  ...
  if (geometry.type === 'MultiPolygon' && Array.isArray(coords)) {
    const largest = (coords as unknown[]).map((polygon) => (Array.isArray(polygon) ? ringToPoints(polygon[0]) : [])).sort((a, b) => b.length - a.length)[0];
    return largest || [];
  }
  return [];
}
...
function geometrySimplification(level: AdminLevel, bbox?: BoundingBox | null): number | null {
  if (!bbox) return null;
...
function buildQueryUrl(level: AdminLevel, where: string, offset: number, bbox?: BoundingBox | null): string {
  ...
  const simplification = geometrySimplification(level, bbox);
  if (simplification !== null) params.set('maxAllowableOffset', String(simplification));
  if (bbox) {
    ...
    params.set('geometryType', 'esriGeometryEnvelope'); params.set('inSR', '4326'); params.set('spatialRel', 'esriSpatialRelIntersects');
  }
...
class OfficialAdministrativeBoundaryService {
  ...
  private async loadLevel(level: AdminLevel, parentCode?: string, bbox?: BoundingBox | null): Promise<AdministrativeFeature[]> {
    ...
    for (let offset = 0; ; offset += MAX_RECORD_COUNT) {
      ...
      all.push(...page); if (json.exceededTransferLimit !== true) break;
    }
    const unique = Array.from(new Map(all.map((f) => [f.adminCode, f])).values()); this.memoryCache.set(key, unique); return unique;
  }
  ...
  async lookupAdministrativeByPoint(point: LatLngPoint): Promise<AdministrativeSpatialLookupResult> {
    ...
    const probe = 0.02; const bbox: BoundingBox = { minLat: point.lat - probe, maxLat: point.lat + probe, minLng: point.lng - probe, maxLng: point.lng + probe };
    const villages = await this.getJawaBaratVillages(bbox); const village = villages.find((feature) => isPointInPolygon(point, feature.coordinates));
