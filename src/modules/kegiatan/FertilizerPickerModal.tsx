  ...
  const renderFertilizerItem = (fert: Fertilizer) => {
    ...
    return (
      <button
        ...
        className={`w-full text-left p-3 rounded-xl border transition-all flex items-start gap-3 active:scale-[0.99] ${
          isSelected
      ...
      >
        {/* Radio Button Indikator Proporsional (Fixed 16px) */}
        <div
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {fert.isSubsidized && (
              <span className="shrink-0 inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-extrabold bg-emerald-100 text-emerald-900 border border-emerald-300">
                Subsidi
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap mt-1 text-xs text-slate-600">
            {fert.formula && (
              <span className="inline-block font-semibold text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded text-[11px] border border-slate-200/70">
                {fert.formula}
              </span>
            )}
      ...
      <div className="space-y-3.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-600"
          />
          {searchQuery && (
            <button
              type="button"
            ...
            <div className="space-y-4">
              ...
              {groupedFertilizers.subsidized.length > 0 && (
                <div className="space-y-1.5">
                  <div className="px-1 flex items-center justify-between text-xs font-bold text-emerald-950 uppercase tracking-wide">
              ...
              {groupedFertilizers.nonSubsidizedInorganic.length > 0 && (
                <div className="space-y-1.5">
                  <div className="px-1 text-xs font-bold text-slate-700 uppercase tracking-wide">
