...
function replaceOnce(file, pattern, replacement, description) {
  ...
  if (next === source) {
    throw new Error(`Patch target not found: ${file} :: ${description}`);
  }
...
replaceOnce(
  ...
  'switch map source to official BIG provider'
);
...
replaceOnce(
  ...
  (match) => {
    if (match.includes('onAdminViewportChange')) return match;
  ...
  'add optional admin viewport hook'
);
...
replaceOnce(
  ...
  'accept admin viewport hook'
);

replaceOnce(
  'src/modules/peta/PetaPertanianView.tsx',
  /  \/\/ Inisialisasi batas wilayah 4-level resmi BIG[\s\S]*?\n  \/\/ GPS State/,
  `  // Inisialisasi batas wilayah resmi BIG: seluruh Jawa Barat.
  ...
  // GPS State`,
  'load complete Jabar hierarchy and viewport villages'
);
...
replaceOnce(
  ...
  /import \{ bigGeospatialService \} from '\.\.\/\.\.\/services\/bigGeospatialService\.ts';/,
  "import { officialAdministrativeBoundaryService } from '../../services/officialAdministrativeBoundaryService.ts';",
  'use official BIG provider for parcel spatial lookup'
);

replaceOnce(
  'src/modules/peta/SaveDrawnParcelModal.tsx',
  /bigGeospatialService\.lookupAdministrativeByPoint\(centroid\)/,
  'officialAdministrativeBoundaryService.lookupAdministrativeByPoint(centroid)',
  'use current BIG boundary lookup'
);
...
replaceOnce(
  ...
  `      mapInstanceRef.current = map;
      ...
      // Pasang Capture Tap Prioritas Tinggi`,
  'emit viewport changes for administrative data'
);
...
replaceOnce(
  ...
  "      detachCapture = attachDrawingMapClickCapture(\n",
  'preserve drawing capture setup'
);
...
replaceOnce(
  ...
  'test official provider'
);

replaceOnce(
  'tests/administrative-boundary-completeness.test.ts',
  /bigGeospatialService\./g,
  'officialAdministrativeBoundaryService.',
  'redirect completeness test to official provider'
);
